...
void main(void) {
    ...
    while (1) {
        if (play) {  // Only run the pattern when play is enabled
            // Loud (high frequency)
...
void switch_interrupt_handler() {
    ...
    if (!(p2val & SW1)) {          // Check if Sw1 (P2.0) is pressed (active low)
        ...
        if (play) {
            P1OUT |= LED_RED;      // Turn on red LED to indicate play
            P1OUT &= ~LED_GREEN;
        } else {
            P1OUT |= LED_GREEN;    // Turn on green LED to indicate pause
            P1OUT &= ~LED_RED;
        }
    }
